      ...
      <table class="table m-auto wrapper">
        ...
        <c:forEach items="${topNotions}" var="notion">
          <tr>
            ...
            <td><a class='linkToNotion' href='${root}/notion/${notion.id}'>${notion.title}</a></td>
      ...
      <nav aria-label="Page navigation example">
        <ul class="pagination">
          ...
          <c:if test="${page.prev}">
            <li class="page-item">
              <a class="page-link"
                 href="${root}/notion/list?pageNum=${page.startPage-1}&amount=${page.amount}">이전</a>
            </li>
          </c:if>
          <%-- 이전버튼종료 --%>
          <%-- 페이징번호 처리시작 --%>
          <c:forEach var="num" begin="${page.startPage}" end="${page.endPage}">
            <li class="page-item">
              <a class="page-link" href="${root}/notion/list?pageNum=${num}&amount=${page.amount}">${num}</a>
            </li>
          </c:forEach>
          <%-- 페이징번호 처리종료 --%>
          <%-- 시작버튼시작 --%>
          <c:if test="${page.next}">
            <li class="page-item">
              <a class="page-link"
                 href="${root}/notion/list?pageNum=${page.endPage + 1}&amount=${page.amount}">다음</a>
            </li>
          </c:if>
          <%-- 시작버튼종료 --%>
        </ul>
